class MemoryGame extends React.Component{
  constructor(props) {
    ...
    this.state = {
          ...
          firstStart: true,
          name: "KI",
          highScores: [{name: "KI 1", score: 40}, {name: "KI 2", score: 60}, {name: "KI 3", score: 1000}],
          testText: "init",
          myImg = new
    };
  ...
  checkHighScore(){
    for (let i = 0; i < this.state.highScores.length; i++) {
      if(this.state.score <= this.state.highScores[i].score){
        ...
        this.state.highScores[i].name = nameInput;
        break;
      }
    }
    this.state.clickCount = 0;
...
/*     const request = new XMLHttpRequest();
    const url = 'http://localhost:3000/memHighscore';
    request.open("GET", url);
    request.send();
    
    request.onload = (e) => {
        alert(request.response);
        this.state.testText = request.response;
        ...
        this.state.highScores = request.response;
        this.setState( { update:0, update:1}); //update rendering
    } */
    // https://stackoverflow.com/questions/14528385/how-to-convert-json-object-to-javascript-array
  const url = 'http://localhost:3000/memHighscore';
  //const url = 'https://animememory.herokuapp.com/memHighscore';
  fetch(url)
  .then(data => data.json())
  .then((json) => {
      alert(JSON.stringify(json));
      this.state.testText = JSON.stringify(json);
      this.state.highScores = JSON.stringify(json);
      var as = JSON.parse( JSON.stringify(json) );
      console.log(as);
      this.state.highScores = as
      this.setState( { update:0, update:1}); //update rendering
  });
  console.log(this.state.testText);
  //console.log(highs);
  //this.state.testText = "request.response";
  //             http://localhost:3000/memHighscore
};
  ...
  placeCards(){
    ...
    for(let i = 0; i < 46; i++){
        imageNames.push(cardList.cardList.src)
    }
    // an array of images 
    var images = [];
...
class PlayField extends React.Component {
  placeCardsOnTable(props) {
      ...
      const listItems = numbers.map((number)  =>
        <button className="memory-playingcard" key={number.toString()} value={number.toString()} onClick={props.click} style={{ backgroundImage: "url('" + props.backgrund[number].src + "')"}} >
        </button>
      );
      return (
      <ul>{listItems}</ul>

class MemoryGame extends React.Component{
  constructor(props) {
    ...
    this.getHighScoreServer_TEST = this.getHighScoreServer_TEST.bind(this);
    this.testHighScore = this.testHighScore.bind(this);
    //dbTest
    this.dbOpen = this.dbOpen.bind(this);
    this.dbClose = this.dbClose.bind(this);
  ...
  clickOnCard(e){
    ...
    if(this.state.score ==0){
      for(let i = 0; i < 20; i++){
        ...
        //this.state.cardImageOnTable[i].src = this.state.topCard.Src;
        //this.state.cardImageOnTable[i].src = this.state.cardList.getCard(4).src;
      }
    }
  ...
  checkHighScore(){
    for (let i = 0; i < this.state.highScores.length; i++) {
      if(this.state.score <= this.state.highScores[i].score){
        ...
        socket.emit('updatehighScore', packet);
        console.log("packet send to highsore fired---------");
        this.setState( { update:0, update:1}); //update rendering
        break;
      }
    }
    this.getHighScoreServer_TEST();
    // this.state.clickCount = 0;
    this.setState( { update:0, update:1}); //update rendering
  }
  ...
  */
  newGame(){
    //this.testRestApi(); //teste REST
    //this.getHighScoreServer_TEST(); //old
    // this.testHighScore(); //new show higscore
  ...
  getHighScoreServer_TEST(){
    console.log(" gethighScore fired ");
    socket.emit('gethighScore', "a"); //test SQL get higscor
    //handle answer 
    ...
    //test SQL set higscor
    // const packet = {score: this.state.score, name: this.state.name};
    // console.log("packet send highsore ---------");
    // console.log(packet);
    // socket.emit('updatehighScore', packet);
    //console.log("Socket emit " + socket.emit('highScore', "this.playerDataPacket"));
  }

  synchroHighScore(){ //ist button getHisgcores
    // this.testSetNewHighScore();
    socket.emit('gethighScore', "a"); //test SQL get higscor
    //handle answer 
    socket.on('gethighScore', data => {
      console.log("socket gethighScore fired ");
  ...
  testHighScore(){ //ist button getHisgcores
    // this.testSetNewHighScore();
    this.testGetHighScore();
    this.setState( { update:0, update:1}); //update rendering
  }
  ...
  testSetNewHighScore(){
    //test SQL set higscor
    console.log("updatehighScore fired ");
    const packet = {score: 32, name: this.state.name};
    console.log("packet send to highsore ---------");
    console.log(packet);
    socket.emit('updatehighScore', packet);
    this.setState( { update:0, update:1}); //update rendering
  }
  ...
  placeCards(){
    ...
    for(let i = 0; i < 20; i = i+2){
      //const tmpCard = this.state.cardList.getCard(i)
      ...
      //this.state.imgArray[i] = new Image();
      //this.state.imgArray[i].src = tmpCard.src;
      //this.state.imgArray[i+1] = new Image();
      //this.state.imgArray[i+1].src = tmpCard.src;
    }
    //preload images
    for(let i = 0; i < 20; i++){
      var element  = document.getElementById(i);
  ...
  dbWrite(){
    const packet = {score: 35, name: this.state.name};
    console.log("packet send to highsore ---------");
    console.log(packet);
    socket.emit('updatehighScore', packet);
  }
    ...
    return (
      <div>
        ...
        <div>
         ...
         {/* <button className="highScoreButton" id="highScoreButton" onClick={this.dbOpen}>open </button>
          <button className="highScoreButton" id="highScoreButton" onClick={this.dbClose}>close </button>
          <button className="highScoreButton" id="highScoreButton" onClick={this.dbWrite}>write </button>
          <button className="highScoreButton" id="highScoreButton" onClick={this.dbRead}>read </button>    */}
        </div>      
